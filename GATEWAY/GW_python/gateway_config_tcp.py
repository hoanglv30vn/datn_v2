...
class udp (threading.Thread, Ui_MainWindow):
    ...
    def thongtincauhinhnode(self,data):
        ...
        soluongcambien = 0
        soluongthietbi = 0    
        connect_sql = sqlite3.connect('data_config.db')   #kết nối tới database
    ...
    def update_state_thietbi(self, data):
        ...
        connect_sql = sqlite3.connect('data_config.db')   #kết nối tới database
        cursor_ = connect_sql.cursor()        
        cursor_.execute("SELECT ID_THIETBI FROM DATA_NODE WHERE ID_NODE = ? AND PHANLOAI = ? ", [id_node_upload_state,"Thiết bị"] )          
    ...
    def uploadDataSensor(self, data):
        ...
        connect_sql = sqlite3.connect('data_config.db')   #kết nối tới database
        cursor_ = connect_sql.cursor()
        cursor_.execute("SELECT ID_THIETBI FROM DATA_NODE WHERE ID_NODE = ? AND PHANLOAI = ? ", [id_node_upload_sensor,"Cảm biến"] )  
